...
class DailyRules:
    ...
    cooldown_minutes_after_loss: int | None = None  # block new trades for N min after a loss
    ...
    def label(self) -> str:
        ...
        if self.cooldown_minutes_after_loss:
            bits.append(f"cd{self.cooldown_minutes_after_loss}")
        return "_".join(bits)
...
class DayState:
    ...
    pnl: float = 0.0
    locked: bool = False
    cooldown_until: object = None     # pd.Timestamp | None, set after a loss
...
def admit_trade(rules: DailyRules, day: DayState, ts) -> bool:
    ...
    if not session_in(rules.session_only, ts):
        return False
    if day.cooldown_until is not None and ts < day.cooldown_until:
        return False
    return True


def update_day(rules: DailyRules, day: DayState, dollar_pnl: float,
               *, ts=None) -> None:
    """Mutate `day` after a trade closes.

    `ts` is the trade's exit timestamp; required when
    `cooldown_minutes_after_loss` is set so the next-allowable time
    can be recorded.
    """
    day.n_trades += 1
    day.pnl += dollar_pnl
    ...
    if rules.daily_loss_stop_dollar is not None and day.pnl <= -rules.daily_loss_stop_dollar:
        day.locked = True
    if (rules.cooldown_minutes_after_loss is not None
            and dollar_pnl < 0 and ts is not None):
        import pandas as _pd  # local import; module otherwise tz-agnostic
        day.cooldown_until = _pd.Timestamp(ts) + _pd.Timedelta(
            minutes=int(rules.cooldown_minutes_after_loss))
...
def all_rule_sets() -> list[DailyRules]:
    return [
        ...
        DailyRules(name="cd60",          cooldown_minutes_after_loss=60),
        DailyRules(name="cd120",         cooldown_minutes_after_loss=120),
    ]
